...
<div class="comment flex-column" onclick={() => updateViewportComponent(7, { id: release != null ? release?.id : comment.release.id } )}>
    <div class="comment-author flex-row" onclick={(e) => e.stopPropagation()}>
        <div class="comment-author-image" onclick={() => updateViewportComponent(8, comment.profile.id)}>
            <img height="40" width="40" src={comment.profile.avatar} alt={comment.profile.login} />
        </div>
        <div class="comment-author-info flex-row">
            <div class="flex-column">
                <div class="comment-author-name flex-row">
                    <div class="comment-author-username flex-row" onclick={() => updateViewportComponent(8, comment.profile.id)}>
                    ...
                    <span class="rel" class:rel-text={!comment.profile?.badge_url}>к релизу</span>
                </div>
                <div class="title-name">
                    <span>{release != null ? release?.title_ru : comment.release.title_ru}</span>
                </div>
            </div>
        </div>
    </div>
